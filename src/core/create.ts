...
function stripFormats(delta: Delta): Delta {
	return new Delta({
		ops: delta.ops.map((op) =>
			typeof op.insert === "string" ? { insert: op.insert } : op,
		),
	});
}
...
class EditorFacade implements EditorInstance {
	...
	constructor(options: EditorOptions) {
		...
		this.variable = variables?.length
			? new VariableBinding(this.editor, variables)
			: null;
	...
	private patchClipboard(): void {
		...
		clipboard.convert = (input, formats) => {
			const sanitized = this.sanitizeText(
				stripFormats(originalConvert(input, formats)),
			);
	...
	private getContentLength(): number {
		...
		return this.variable
			? this.variable.measureText(text)
			: text.replace(/\r\n|\r|\n/g, "").length;
	}
